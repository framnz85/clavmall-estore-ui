...
const CodCheckout = ({succeeded, setSucceeded}) => {
  ...
  return (
    <>
      ...
      {succeeded && <><div className="alert alert-success" role="alert">
        {payopt.details.map(det => (
          <div align="center" key={det._id}>
            <b>{det.desc}</b><br/>{det.value}
          </div>
        ))}
      </div><br /></>}
