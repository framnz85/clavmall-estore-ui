...
const PaypalCheckout = ({succeeded, setSucceeded}) => {
    ...
    const { payopt } = inputs;
    const currencyCode = currencySymbols.filter(cur => cur.country === estore.country.name);
    ...
    const handleApprove = (sellerTxnID) => {
        if (sellerTxnID) {
            createOrder({ paymentOption: payopt, orderCode, sellerTxnID }, user.token).then((res) => {
                if (res.data.ok) {
                    dispatch({
    ...
    return (
        <>
            ...
            {!succeeded && <>
                {clientId && !succeeded && <PayPalScriptProvider options={{
                    "client-id": clientId,
                    currency: currencyCode[0] ? currencyCode[0].code : "USD",
                }}>
                    <PayPalButtons
                        onClick={(data, actions) => {
                            grandTotal === 0 && toast.error("This order is no longer valid");
                        }}
